...
采用 **iframe + postMessage** 架构：首页作为外壳容器通过 iframe 加载子页面，底部音乐播放器跨页面持续播放，不中断。
...
- **iframe 主题同步**：主题切换通过 postMessage 广播到所有 iframe 子页面，统一亮/暗模式
- **侧导航**：固定右侧圆点导航，hover 展开显示标签，简洁过渡无闪烁
- **音乐栏回收动画**：先隐藏文字再执行容器收缩动画，全程文字不可见
- **导航栏对齐**：下拉菜单项垂直对齐修复，品牌链接跳转，左右布局优化
